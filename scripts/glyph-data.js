...
(function (root) {
    ...
    "loop": {
      ...
      "constraints": [
        { "id": "no-coarsen", "forbid": "@coarsen", "severity": "ask", "label": "regride o detalhe",
          ...
          "suggest": "elabore dentro da passagem; resuma ou generalise depois que [cond] fechar o laço, não no meio dele" },
  ...
  root.GlyphRules = {
  "schema": 2,
  "note": "Semantic rule table. Data, not code: editing this file changes engine behaviour. Four rule kinds. `pair` fires when both commands land on the same target (siblings, or one an ancestor of the other). `order` fires when `then` appears before `first` among siblings. `precondition` fires when `target` is not preceded by any of `requiresBefore` in the same segment. Names prefixed with @ resolve through `classes`. Every rule carries a `suggest`: the repair shape, not just the complaint. `blend` is the pattern layer: it matches co-occurrence in the BURNT form and EMITS a richer element instead of diagnosing. It is written over the atoms, so a pattern is invariant to which surface synonym the author typed. A blend with no `means` is refused at compile time — an invented element that cannot say what it means moves the interpretation problem one step along instead of solving it.",
  "classes": {
    "thinking": [
      "CLAR",
      "CRIT",
  ...
  "rules": [
    ...
    },
    {
      ...
      "note": "`DENY` bears on the route to a result and `REQ` on the existence of something, so the pair is a tension and not a contradiction — severity `ask`, label `tension`. The record of the demotion is .guidelines/.history/GLOSSARY_CLOSED.md §6.6."
    },
    {
      "id": "req-dont",
      "kind": "pair",
  ...
  "notRules": [
    {
      "pair": "CMP + DIST",
      "why": "comparing and distinguishing are complementary, not opposed"
    },
    {
      "pair": "CRIT + IMPR",
      "why": "criticise then improve is the natural work sequence"
    },
    {
      "pair": "BRST + CAT",
      "why": "generate items, then group them"
    },
    {
      "pair": "ALW + COND",
      "why": "'always, under this condition' is scoping, not contradiction"
    },
    {
      "pair": "POS + CRIT",
      "why": "criticism can be positive"
    },
    {
      "pair": "EXC + INSTOF",
      "why": "exception and substitution coexist — FUSIONS_WORKSHEET.md, house 7"
    }
  ]
};
