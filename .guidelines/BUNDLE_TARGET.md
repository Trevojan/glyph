...
**I1b — the generated/authored line can run through a FILE or through a FIELD.**
I1 is file-granular, and that holds only while every field of a generated file is
machine-derived. `expansions.json` is compiled from `expansions.txt` and
`GLOSSARY.md`; the first time a command needs a human annotation, a file-level
rule offers two bad answers — a second file, or lose the annotation on the next
build.

The answer, taken from `dmh-protocol`'s indexer and recorded before it is needed:
**a regeneration refreshes the machine-derived fields and preserves the
human-owned ones, matched by content identity rather than by position.** A
generated file that carries authored fields declares which are which, and the
banner says *do not edit these*, not *do not edit this file*.

Not implemented, and deliberately so: no store carries an authored field today.
It is written down so the answer is not improvised under pressure the first time
one does.
